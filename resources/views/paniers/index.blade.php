            ...
            <div class="col-md-8 cart">
                <div class="title">
                    <div class="row">
                        ...
                        <div class="col align-self-center text-right text-muted">{{Cart::count()}} articles</div>
                    </div>
                </div>    
                <div class="row border-top border-bottom">
                    @foreach(Cart::content() as $article)
                    <div class="row main align-items-center">
                        <div class="col-2"><img class="img-fluid" src="{{ $article->model->image }}"></div>
                        <div class="col">
                            <div class="row text-muted">{{ $article->model->nom }}</div>
                            <div class="row">{{ $article->model->nom }}</div>
                        </div>
                        <div class="col">
                            <a href="#">-</a><a href="#" class="border">1</a><a href="#">+</a>
                        </div>
                        <div class="col">{{ $article->model->getPrice() }} 
                            <form action="{{ route('paniers.destroy', $article->rowId)}}" method="POST">
                                @csrf
                                @method('DELETE')
